...
async def predict_text(
    ...
    try:
        ...
        # Blacklist check
        blacklist_check = check_blacklist(company=company_profile)
...
class ALLPDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'FakeJobAI - Analysis Report', 0, 1, 'C')
        self.ln(5)

    def footer(self):
        self.set_y(-15)
        self.set_font('Arial', 'I', 8)
        self.cell(0, 10, 'Generated by FakeJobAI - Page ' + str(self.page_no()), 0, 0, 'C')
...
async def generate_report(data: dict = Body(...)):
    try:
        ...
        pdf.cell(0, 10, f"Job Profile: {title}", 0, 1)
        pdf.set_font("Arial", '', 12)
        pdf.cell(0, 8, f"Company: {company}", 0, 1)
        pdf.cell(0, 8, f"Analysis Date: {datetime.now().strftime('%Y-%m-%d %H:%M')}", 0, 1)
        pdf.ln(10)
        ...
        res_color = "red" if "Fake" in prediction else "green"
        pdf.set_text_color(200 if "Fake" in prediction else 0, 100 if "Real" in prediction else 0, 0)
        pdf.cell(0, 8, f"Verdict: {prediction} ({confidence} Confidence)", 0, 1)
        pdf.set_text_color(0,0,0) # Reset
        
        pdf.cell(0, 8, f"Risk Score: {risk_score}/100", 0, 1)
        pdf.cell(0, 8, f"Risk Level: {str(risk_level).upper()}", 0, 1)
        pdf.ln(5)
        
        # Explanation
        if explanation and explanation.get("ai_summary"):
            pdf.set_font("Arial", 'B', 14)
            pdf.cell(0, 10, "AI Logic Summary", 0, 1)
            pdf.set_font("Arial", '', 11)
            pdf.multi_cell(0, 6, explanation.get("ai_summary"))
            pdf.ln(5)
            
        # Disclaimer
        pdf.ln(10)
        pdf.set_font("Arial", 'I', 8)
        pdf.multi_cell(0, 5, "Disclaimer: This report is generated by an AI model. While we strive for accuracy, false positives/negatives can occur. Always verify directly with the company.")
        
        # Save to temp
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            pdf.output(tmp.name)
            return FileResponse(tmp.name, filename=f"JobGuard_Report_{int(datetime.now().timestamp())}.pdf", media_type="application/pdf")
...
async def get_analytics():
    ...
    try:
        ...
        cursor = conn.execute("SELECT result, COUNT(*) FROM predictions GROUP BY result")
        breakdown = {row[0]: row[1] for row in cursor.fetchall()}
        
        # Risk level distribution
        cursor = conn.execute("SELECT risk_level, COUNT(*) FROM predictions WHERE risk_level IS NOT NULL GROUP BY risk_level")
        ...
        cursor = conn.execute("""
            SELECT DATE(timestamp) as date, 
                   ...
                   SUM(CASE WHEN result LIKE '%Real%' THEN 1 ELSE 0 END) as real
            FROM predictions 
            WHERE timestamp >= datetime('now', '-7 days')
            GROUP BY DATE(timestamp)
            ORDER BY date
        """)
        trend = [{"date": row[0], "fake": row[1], "real": row[2]} for row in cursor.fetchall()]
        
        # Feedback stats
        cursor = conn.execute("SELECT COUNT(*), SUM(user_says_correct) FROM feedback")
        feedback_row = cursor.fetchone()
        feedback_total = feedback_row[0] or 0
        feedback_correct = feedback_row[1] or 0
        ...
        return JSONResponse({
            ...
            "feedback": {
                ...
                "accuracy": round((feedback_correct / feedback_total) * 100, 1) if feedback_total > 0 else 0
            },
            "reports": {
                "total": total_reports
            },
...
async def register_visitor(visitor: VisitorRegistration):
    ...
    try:
        ...
        if existing:
            ...
            # New visitor - register and send welcome email
            conn.execute("""
                INSERT INTO visitors (email, name, source, registered_at, last_visit)
                VALUES (?, ?, ?, ?, ?)
            """, (visitor.email, visitor.name or "", visitor.source, now, now))
            conn.commit()
            conn.close()
            
            # Send welcome email
            email_result = {"success": False, "message": "Email service not configured"}
            try:
                from utils.email_service import send_welcome_email
                user_name = visitor.name if visitor.name else visitor.email.split("@")[0].title()
                email_result = send_welcome_email(visitor.email, user_name)
            except Exception as email_error:
                email_result = {"success": False, "error": str(email_error)}
            
            return JSONResponse({
                "success": True,
                "message": "Welcome to FakeJobAI!",
                "returning": False,
                "email_sent": email_result.get("success", False),
                "email_status": email_result.get("message") or email_result.get("error", "")
            })
...
async def send_welcome_email_endpoint(request: WelcomeEmailRequest):
    ...
    try:
        ...
        # Send welcome email (only for new users, or you can send for all)
        email_result = {"success": False, "message": "Email service not configured"}
        
        try:
            from utils.email_service import send_welcome_email
            user_name = request.name if request.name else request.email.split("@")[0].title()
            email_result = send_welcome_email(request.email, user_name)
        except ImportError:
            email_result = {"success": False, "error": "Email service module not found"}
        except Exception as email_error:
            email_result = {"success": False, "error": str(email_error)}
        ...
        return JSONResponse({
            ...
            "email_sent": email_result.get("success", False),
            "email_status": email_result.get("message") or email_result.get("error", ""),
            "message": "Welcome email sent!" if email_result.get("success") else "User registered (email pending)"
        })
